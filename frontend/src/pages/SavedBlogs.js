...
const SavedBlogs = () => {
  ...
  const fetchSavedBlogs = async () => {
    try {
      ...
      console.log('Fetching saved blogs for user:', user.uid);

      // Get all blog IDs that the user has saved
      const savedBlogIds = await saveService.getBlogsSavedByUser(user.uid);
      ...
      // Get the full blog details for saved blogs
      const savedBlogs = [];
      ...
      savedBlogs.sort((a, b) => new Date(b.date || b.createdAt || 0) - new Date(a.date || a.createdAt || 0));
      
      console.log('Final saved blogs count:', savedBlogs.length);
      setBlogs(savedBlogs);
    } catch (error) {
