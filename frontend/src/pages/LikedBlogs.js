...
const LikedBlogs = () => {
  ...
  const fetchLikedBlogs = async () => {
    try {
      ...
      console.log('Fetching liked blogs for user:', user.uid);

      // Get all blog IDs that the user has liked
      const likedBlogIds = await likeService.getBlogsLikedByUser(user.uid);
      ...
      // Get the full blog details for liked blogs
      const likedBlogs = [];
      ...
      likedBlogs.sort((a, b) => new Date(b.date || b.createdAt || 0) - new Date(a.date || a.createdAt || 0));
      
      console.log('Final liked blogs count:', likedBlogs.length);
      setBlogs(likedBlogs);
    } catch (error) {
  ...
  return (
    <div className="blog-collections-container" data-testid="liked-blogs-page">
      <div className="collections-header">
        ...
        <p className="collections-subtitle">
          {blogs.length} blog{blogs.length !== 1 ? 's' : ''} you've liked
        </p>
      </div>
      ...
      {blogs.length === 0 ? (
        <div className="empty-state">
          ...
          <p>Start exploring and like blogs that interest you!</p>
