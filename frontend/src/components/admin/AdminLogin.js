...
const AdminLogin = ({ onLogin }) => {
  ...
  const onGoogleSuccess = async (response) => {
    ...
    try {
      const { email, name, sub: googleId } = response.profileObj;
      ...
      // Send the Google token to backend for verification and JWT creation
      const backendResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/admin/google-login`, {
        method: 'POST',
        headers: {
        ...
        <div className="bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl">
          ...
          <div className="space-y-6">
            ...
            <div className="text-center">
              ...
              {isLoading ? (
                <div className="w-full py-3 px-4 bg-gradient-to-r from-purple-600 to-blue-600 text-white font-semibold rounded-lg flex items-center justify-center opacity-50 cursor-not-allowed">
          ...
          <div className="mt-6 p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
            <p className="text-xs text-blue-400 text-center">
              ...
              Contact the administrator if you need access to this panel.
            </p>
          </div>
        </div>
      </div>
