...
class LikeService {
  ...
  // Get all blog IDs that a user has liked
  async getBlogsLikedByUser(userId) {
    try {
      ...
      // Iterate through all blogs and check if user liked them
      Object.keys(allLikes).forEach(blogId => {
        const blogLikes = allLikes[blogId];
        if (blogLikes && blogLikes[userId]) {
